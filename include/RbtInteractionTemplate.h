...
class RbtInteractionNodeList: public std::vector<RbtInteractionNode<T>*> {};  // std:vector ctors

template <typename T>                                                               // instead RbtInteractionListMap
class RbtInteractionNodeListMap: public std::vector<RbtInteractionNodeList<T>> {};  // std:vector ctors

template <typename T>
class RbtInteractionNodeListMapIter: public RbtInteractionNodeListMap<T>::iterator {};
...
class RbtInteractionGridTemplate: public RbtBaseGrid {
    public:
        ...
        RbtInteractionGridTemplate(const RbtCoord& gridMin, const RbtCoord& gridStep, RbtUInt NX, RbtUInt NY,
        ...
        void SetInteractionLists(T* pIntn, RbtDouble radius) {
            ...
            if (NULL == pAtom) return;
            const RbtCoord& c = pAtom->GetCoords();
            RbtUIntList sphereIndices;
            GetSphereIndices(c, radius, sphereIndices);
            for (RbtUIntListConstIter sphereIter = sphereIndices.begin(); sphereIter != sphereIndices.end();
                 sphereIter++) {
        ...
        void ClearInteractionLists() {
            for (RbtInteractionNodeListMap<T>::iterator iter = m_intnMap.begin(); iter != m_intnMap.end(); iter++) {
                (*iter).clear();
            }
        };
    ...
    private:
        ...
        void CopyGrid(const RbtInteractionGridTemplate<T>& aGrid) { m_intnMap = aGrid.m_intnMap; };
...
class RbtInteractionGridTemplatePtr: public SmartPtr<RbtInteractionGridTemplate<T>> {};
