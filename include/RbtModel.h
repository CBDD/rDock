...
class RbtModel {
    public:
        ...
        // DM 09 Feb 1999 - Rotate around a given bond by theta degrees, keeping the given atom fixed (only spins the
        ...
        RbtUInt GetNumBonds(const Predicate& pred);
        // Generic template version of GetBondList, passing in your own predicate
        template <class Predicate>
        RbtBondList GetBondList(const Predicate& pred);
    ...
    private:
        ...
        RbtStringIntMap
            m_coordNames;  //(DM 8 Feb 1999) map of named coord sets (key=name, value=index into RbtAtom coord map)
