...
void RbtPolarSF::BuildIntraMap(
    ...
    for (RbtInteractionCenterListConstIter iter = ICList1.begin(); iter != ICList1.end(); iter++) {
        ...
        // but we check all the constituent atoms of the IC to assess whether the interaction should be deemed
        // "flexible" Reason is that some interactions have an angular dependence: the primary distance may be rigid,
...
void RbtPolarSF::BuildIntraMap(const RbtInteractionCenterList& ICList, RbtInteractionListMap& intns) const {
...
void RbtPolarSF::Partition(
    ...
    for (RbtInteractionListMapIter iter = prtIntns.begin(); iter != prtIntns.end(); iter++) (*iter).clear();
    if (iTrace > 3) {
...
RbtDouble RbtPolarSF::PolarScore(
    ...
    RbtBool bPlane1 =
        ((pAtom1_2 != NULL) && (pAtom1_3 != NULL) && (eLP1 == RbtInteractionCenter::NONE)) ? true : false;
    ...
    for (RbtInteractionCenterListConstIter IC2Iter = IC2List.begin(); IC2Iter != IC2List.end(); IC2Iter++) {
        ...
        RbtBool bPlane2 =
            ((pAtom2_2 != NULL) && (pAtom2_3 != NULL) && (eLP2 == RbtInteractionCenter::NONE)) ? true : false;
        RbtBool bLP2 =
            ((pAtom2_2 != NULL) && (pAtom2_3 != NULL) && (eLP2 != RbtInteractionCenter::NONE)) ? true : false;
        ...
        if (f > 0.0) {
            // cout << endl << "R12:\t" << pAtom1_1->GetFullAtomName() << "," << pAtom2_1->GetFullAtomName() << "," <<
            // R12
            //	   << "," << DR << ", f=" << f << endl;
            // For guanidinium bPlane2 interacting with C=O lone pair bLP1, we want to use the regular angular
            // dependence
            ...
            // Lone pair geometry dependence around Osp2
                ...
                // cout << "dPerp = " << dPerp << " check cPerp = " << Rbt::DistanceFromPointToPlane(cPerp, pl1) <<
                // endl; Can calculate sin(theta) directly from the two distances we have already
            ...
            if (f > 0.0) {
                // For guanidinium bPlane1 interacting with C=O lone pair bLP2, we want to use the regular angular
                // dependence
                if (bAngle2 || (bPlane1 && bLP2)) {
                    ...
                    const RbtCoord& cAtom2_2 = pAtom2_2->GetCoords();
                    const RbtCoord& cAtom2_3 = pAtom2_3->GetCoords();
                    const f1prms& PHI2prms =
                        (eLP2 == RbtInteractionCenter::LONEPAIR) ? m_PHI_lp_prms : m_PHI_plane_prms;
                    RbtPlane pl2 = RbtPlane(cAtom2_1, cAtom2_2, cAtom2_3);
                    RbtDouble dPerp = Rbt::DistanceFromPointToPlane(cAtom1_1, pl2);
                    RbtCoord cPerp = cAtom1_1 - dPerp * pl2.VNorm();
                    // cout << "dPerp = " << dPerp << " check cPerp = " << Rbt::DistanceFromPointToPlane(cPerp, pl2) <<
                    // endl;
                    RbtDouble theta = asin(dPerp / R) * 180.0 / M_PI;
                    f *= f1(fabs(theta), m_THETAprms);
                    if (f > 0.0) {
                ...
                if (f > 0.0) {
                    ...
                    //             << pAtom2_1->GetFullAtomName() << "\t" << pAtom2_1->GetUser1Value() << "\t" << f <<
                    //             endl;
                    s += pAtom2_1->GetUser1Value() * f;
                    if (bAnnotate) {
                        // DM 6 Feb 2003. Include the charge and receptor density scaling factors in the polar
