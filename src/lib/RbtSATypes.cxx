...
RbtHHSType::eType RbtHHSType::operator()(RbtAtom* anAtom) const {
    ...
    switch (anAtom->GetAtomicNo()) {
        case 1:  // Hydrogen
            ...
            if (bondedAtomList.empty()) return H;  // Isolated hydrogen atom
        ...
        case 7:  // Nitrogen
            ...
            if (bIsCat(anAtom) || (bondedAtomList.size() == 4) || (Rbt::GetNumAtoms(bondedAtomList, bIsCat) > 0)) {
                switch (anAtom->GetHybridState()) {
                    case RbtAtom::SP3:
                        return N_sp3p;
        ...
        case 8:  // Oxygen
            ...
            if (bIsAni(anAtom)) return Om;
            // Neutral oxygens from here on
            // Terminal oxygen bonded to nitrogen (mainly nitro groups)
            if ((anAtom->GetCoordinationNumber(7) == 1) && (anAtom->GetNumBonds() == 1)) return ON;
            switch (anAtom->GetHybridState()) {
                case RbtAtom::SP3:
                ...
                case RbtAtom::TRI:
                    ...
                    bondedAtomList = Rbt::GetBondedAtomList(anAtom);
                    if ((Rbt::GetNumAtoms(bondedAtomList, bIsLipo) + nH) == 2) return (nH > 0) ? OH_sp3 : O_sp3;
...
RbtString RbtHHSType::Type2Str(RbtHHSType::eType aType) const { return m_typeNames[aType]; }
    ...
    if (d > 0) {
        ...
        for (HHS_SolvationRListConstIter iter = m_var.begin(); iter != m_var.end(); iter++) {
            if (Rbt::Length2((*iter)->atom->GetCoords(), atom->GetCoords()) < dd) {
                ...
                // cout << "Partitioned " << (*iter)->atom->GetFullAtomName() << " - " << atom->GetFullAtomName() <<
                // endl;
            }
        }
    } else {
...
RbtBool Rbt::isHHSSelected::operator()(const HHS_Solvation* pHHS) const { return pHHS->GetAtom()->GetSelectionFlag(); }
