...
RbtRealGrid::RbtRealGrid(istream& istr): RbtBaseGrid(istr), m_grid(NULL), m_data(NULL) {
...
// Copy constructor
RbtRealGrid::RbtRealGrid(const RbtRealGrid& grid): RbtBaseGrid(grid), m_grid(NULL), m_data(NULL) {
...
RbtRealGrid::RbtRealGrid(const RbtBaseGrid& grid): RbtBaseGrid(grid), m_grid(NULL), m_data(NULL) {
    CreateArrays();
    SetAllValues(0.0);
    _RBTOBJECTCOUNTER_COPYCONSTR_("RbtRealGrid");
}
...
void RbtRealGrid::ReplaceValueRange(RbtDouble oldValMin, RbtDouble oldValMax, RbtDouble newVal) {
    for (RbtUInt i = 0; i < GetN(); i++) {
        float d = m_data[i];
        if ((d >= oldValMin) && (d < oldValMax)) m_data[i] = newVal;
    }
}
...
void RbtRealGrid::SetAccessible(RbtDouble radius, RbtDouble oldVal, RbtDouble adjVal, RbtDouble newVal,
    ...
    RbtUIntList sphereIndices;
    RbtUInt nMax = (int(radius / GetGridStep().x) + 1) * (int(radius / GetGridStep().y) + 1)
...
RbtDouble RbtRealGrid::MinValue() const {
    float fMin = m_data[0];
    for (RbtUInt i = 0; i < GetN(); i++) {
        if (m_data[i] < fMin) fMin = m_data[i];
    }
    return fMin;
}
...
RbtDouble RbtRealGrid::MaxValue() const {
    float fMax = m_data[0];
    for (RbtUInt i = 0; i < GetN(); i++) {
        if (m_data[i] > fMax) fMax = m_data[i];
    }
    return fMax;
}
...
RbtUInt RbtRealGrid::FindMinValue() const {
    RbtUInt iMin = 0;
    for (RbtUInt i = 0; i < GetN(); i++) {
        if (m_data[i] < m_data[iMin]) iMin = i;
    }
    return iMin;
}
...
RbtUInt RbtRealGrid::FindMaxValue() const {
    RbtUInt iMax = 0;
    for (RbtUInt i = 0; i < GetN(); i++) {
        if (m_data[i] > m_data[iMax]) iMax = i;
    }
    return iMax;
}
...
void RbtRealGrid::PrintInsightGrid(ostream& s) const {
    ...
    s << setw(8) << GetGridStep().x * (GetNX() - 1) << setw(8) << GetGridStep().y * (GetNY() - 1) << setw(8)
      << GetGridStep().z * (GetNZ() - 1) << setw(8) << 90.0 << setw(8) << 90.0 << setw(8) << 90.0 << endl;
    // s.setf(0,ios_base::floatfield);
    s << setw(5) << GetNX() - 1 << setw(5) << GetNY() - 1 << setw(5) << GetNZ() - 1 << endl;
    s << setw(5) << 1 << setw(5) << GetnXMin() << setw(5) << GetnXMax() << setw(5) << GetnYMin() << setw(5)
      << GetnYMax() << setw(5) << GetnZMin() << setw(5) << GetnZMax() << endl;
    s.precision(10);
    // s.setf(ios_base::fixed,ios_base::floatfield);
    // s.setf(ios_base::right,ios_base::adjustfield);
