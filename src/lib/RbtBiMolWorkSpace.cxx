...
RbtBiMolWorkSpace::RbtBiMolWorkSpace(): RbtWorkSpace(2) { _RBTOBJECTCOUNTER_CONSTR_("RbtBiMolWorkSpace"); }

RbtBiMolWorkSpace::~RbtBiMolWorkSpace() { _RBTOBJECTCOUNTER_DESTR_("RbtBiMolWorkSpace"); }
...
void RbtBiMolWorkSpace::UpdateModelCoordsFromChromRecords(RbtBaseMolecularFileSource* pSource, RbtInt iTrace) {
            ...
            if (spChrom.Ptr()) {
                ...
                if (chromLength > 0) {
                    ...
                    if (pSource->isDataFieldPresent(chromField)) {
                        ...
                        RbtString chromRecord =
                            Rbt::ConvertListToDelimitedString(pSource->GetDataValue(chromField), ",");
                        // Now split into string values and convert to doubles
                        RbtStringList chromValues = Rbt::ConvertDelimitedStringToList(chromRecord, ",");
                        if (chromLength == chromValues.size()) {
                            RbtDoubleList chromVec;
                            for (RbtStringListConstIter iter = chromValues.begin(); iter != chromValues.end();
                                 ++iter) {
                            ...
                            cout << chromField << " record in " << pSource->GetFileName() << " has "
                                 << chromValues.size() << " elements" << endl;
...
void RbtBiMolWorkSpace::Save(RbtBool bSaveScores) { SaveLigand(GetSink(), bSaveScores); }

// Model I/O
// Saves ligand to history file sink
void RbtBiMolWorkSpace::SaveHistory(RbtBool bSaveScores) { SaveLigand(GetHistorySink(), bSaveScores); }
