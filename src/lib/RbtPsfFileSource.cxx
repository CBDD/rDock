...
RbtPsfFileSource::RbtPsfFileSource(const RbtString& fileName, const RbtString& strMassesFile, RbtBool bImplHydrogens):
    RbtBaseMolecularFileSource(fileName, "PSF_FILE_SOURCE"),  // Call base class constructor
    ...
    if (!m_bParsedOK) {
        ...
        try {
            ...
            while ((m_atomList.size() < nAtomRec) && (fileIter != fileEnd)) {
                ...
                istr >> nAtomId >> strSegmentName >> strSubunitId >> strSubunitName >> strAtomName >> strFFType
...
void RbtPsfFileSource::SetupAtomParams() throw(RbtError) {
    ...
    if (m_bImplHydrogens) RemoveNonPolarHydrogens();
    SetupVdWRadii();
