...
// Constructors
RbtCharmmTypesFileSource::RbtCharmmTypesFileSource(const char* fileName): RbtBaseFileSource(fileName) {
    _RBTOBJECTCOUNTER_CONSTR_("RbtCharmmTypesFileSource");
}

RbtCharmmTypesFileSource::RbtCharmmTypesFileSource(const RbtString fileName): RbtBaseFileSource(fileName) {
    _RBTOBJECTCOUNTER_CONSTR_("RbtCharmmTypesFileSource");
}
        ...
        try {
            ...
            for (RbtFileRecListIter fileIter = m_lineRecs.begin(); fileIter != m_lineRecs.end(); fileIter++) {
                if ((*fileIter).substr(0, 4) == strMassKey) {
                    ...
                    istr >> strDummy >> chrmType.nAtomType >> chrmType.strAtomType >> chrmType.mass
                        >> chrmType.element;
                    //				  >> strDummy
                    //				  >> chrmType.comment;
