...
RbtSegmentMap RbtBaseMolecularFileSource::GetSegmentFilterMap() { return m_segmentFilterMap; }
...
void RbtBaseMolecularFileSource::ClearSegmentFilterMap() { m_segmentFilterMap.clear(); }

RbtBool RbtBaseMolecularFileSource::isSegmentFilterMapDefined() { return !m_segmentFilterMap.empty(); }
...
void RbtBaseMolecularFileSource::RemoveAtom(RbtAtomPtr spAtom) {
    ...
    for (const auto& mapIter: bondMap) {
        ...
        if (bIter != m_bondList.end()) bondIterators.push_back(bIter);
    }

    // and then remove them
    for (auto bIter: bondIterators) {
        DEBUG_ERR(
            "Removing bond #" << (*bIter)->GetBondId() << " (" << (*bIter)->GetAtom1Ptr()->GetAtomName() << "-"
...
void RbtBaseMolecularFileSource::SetupPartialIonicGroups(RbtAtomList& atoms, RbtParameterFileSourcePtr spParamSource) {
    ...
    RbtString match = leadAtom->GetSegmentName() + ":" + subunitName + "_" + leadAtom->GetSubunitId() + ":";
    ...
    if (std::find(atList.begin(), atList.end(), _MANDATORY) != atList.end()) {
        ...
        if (nPresent != mandAtoms.size()) {
            DEBUG_ERR(
                "INFO SetupPartialIonicGroups: Only " << nPresent << " out of " << mandAtoms.size()
    ...
    if (std::find(atList.begin(), atList.end(), _FORBIDDEN) != atList.end()) {
        ...
        if (nPresent > 0) {
            DEBUG_ERR(
                "INFO SetupPartialIonicGroups: " << nPresent << " forbidden atoms present in atom list headed by "
                                                 << leadAtom->GetFullAtomName() << endl
            );
            return;
        }
    ...
    for (RbtStringListConstIter aIter = atList.begin(); aIter != atList.end(); aIter++) {
        if (*aIter != "") {
            ...
            for (RbtAtomListIter iter = selectedAtoms.begin(); iter != selectedAtoms.end(); iter++) {
                ...
                DEBUG_ERR(
                    "INFO Setting group charge on " << (*iter)->GetFullAtomName() << " to " << partialCharge << endl
                );
            }
        }
    }
