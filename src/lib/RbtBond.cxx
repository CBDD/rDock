...
RbtBond::RbtBond():
    ...
    m_spAtom1(),
    m_spAtom2(),
    m_nFormalBondOrder(1),
    m_dPartialBondOrder(1.0),
    m_bCyclic(false),
    m_bSelected(false) {
...
RbtBond::RbtBond(RbtInt nBondId, RbtAtomPtr& spAtom1, RbtAtomPtr& spAtom2, RbtInt nFormalBondOrder):
    m_nBondId(nBondId),
    m_spAtom1(spAtom1),
    m_spAtom2(spAtom2),
...
ostream& operator<<(ostream& s, const RbtBond& bond) {
    ...
    return s << "Bond ID=" << bond.m_nBondId << ", Atoms " << spAtom1->GetAtomId() << "-" << spAtom2->GetAtomId()
...
// Returns bond length
RbtDouble RbtBond::Length() const { return Rbt::BondLength(m_spAtom1, m_spAtom2); }
...
RbtBool Rbt::isBondRotatable::operator()(RbtBond* pBond) const {
    ...
    if (pBond->GetCyclicFlag()) return false;

    // Eliminate non-single bonds
    if (pBond->GetFormalBondOrder() > 1) return false;
    ...
    if (spAtom1->GetCyclicFlag() && spAtom2->GetCyclicFlag()) return true;
    ...
    if ((bIsC_SP2(spAtom1) && !spAtom1->GetCyclicFlag() && bIsN_TRI(spAtom2))
        ...
        return false;
    // Nitro groups
    if ((bIsC_SP2(spAtom1) && bIsN_SP2P(spAtom2)) || (bIsC_SP2(spAtom2) && bIsN_SP2P(spAtom1))) return false;
    ...
    if ((spAtom1->GetNumBonds() == 1) || (spAtom2->GetNumBonds() == 1)) return false;
...
void Rbt::SetBondSelectionFlags(RbtBondList& bondList, RbtBool bSelected) {
    for (RbtBondListIter iter = bondList.begin(); iter != bondList.end(); iter++) (*iter)->SetSelectionFlag(bSelected);
}

// Cyclic bonds
void Rbt::SetBondCyclicFlags(RbtBondList& bondList, RbtBool bCyclic) {
    for (RbtBondListIter iter = bondList.begin(); iter != bondList.end(); iter++) (*iter)->SetCyclicFlag(bCyclic);
}
