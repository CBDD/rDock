...
RbtChromPositionElement::RbtChromPositionElement(const RbtModel* pModel, const RbtDockingSite* pDockSite,
    ...
    m_spRefData = new RbtChromPositionRefData(pModel, pDockSite, transStepSize, rotStepSize, transMode, rotMode,
                                              maxTrans, maxRot);
    SyncFromModel();
    _RBTOBJECTCOUNTER_CONSTR_(_CT);
}

RbtChromPositionElement::RbtChromPositionElement(RbtChromPositionRefDataPtr spRefData, const RbtCoord& com,
                                                 const RbtEuler& orientation):
    m_spRefData(spRefData), m_com(com), m_orientation(orientation) {
    _RBTOBJECTCOUNTER_CONSTR_(_CT);
}

RbtChromPositionElement::~RbtChromPositionElement() { _RBTOBJECTCOUNTER_DESTR_(_CT); }
...
void RbtChromPositionElement::SyncFromModel() { m_spRefData->GetModelValue(m_com, m_orientation); }

void RbtChromPositionElement::SyncToModel() { m_spRefData->SetModelValue(m_com, m_orientation); }
    ...
    if (VectorOK(v, i)) {
        ...
        if (!m_spRefData->IsRotFixed()) {
            ...
            if (orientationElement.size() == 3) {
                ...
                // for tethered bounds
                m_orientation = RbtEuler(orientationElement[0], orientationElement[1], orientationElement[2]);
            } else {
    ...
    if (VectorOK(v, i)) {
        ...
        if (!m_spRefData->IsRotFixed()) {
            ...
            if (rotStepSize > 0.0) {
                ...
                // q.s = cos(phi / 2)
                RbtQuat qAlign = otherOrientation.ToQuat() * m_orientation.ToQuat().Conj();
