...
RbtMOEGridPoint::RbtMOEGridPoint(vector<double> a_coord, double a_value) {
    ...
    for (crditer = a_coord.begin(); crditer != a_coord.end(); ++crditer) coords.push_back(*crditer);
    value = a_value;
}
...
RbtMOEGridShape::RbtMOEGridShape(vector<double> grid_origin, vector<double> grid_extents, double grid_steps) {
    ...
    for (o_iter = grid_origin.begin(); o_iter != grid_origin.end(); ++o_iter) origin.push_back(*o_iter);
    for (e_iter = grid_extents.begin(); e_iter != grid_extents.end(); ++e_iter) extents.push_back(*e_iter);
    step_size = grid_steps;

    // calculate sizes for all vertices of the grid cube
    for (o_iter = origin.begin(), e_iter = extents.begin(); o_iter != origin.end() && e_iter != extents.end();
         ++o_iter, ++e_iter) {
        i_extents.push_back((int)(((*e_iter) - (*o_iter)) / step_size) + 1);
    }
    // calculate the total number of points (initialize multiplication by 1)
    data_size = 1;
    for (vector<int>::const_iterator ext_iter = i_extents.begin(); ext_iter != i_extents.end(); ++ext_iter)
        data_size *= (*ext_iter);
}
...
void RbtMOEGrid::CalculateCommonExtents(vector<RbtString> strPrmFiles) {
    ...
    for (strPrmFilesIter = strPrmFiles.begin(); strPrmFilesIter != strPrmFiles.end(); ++strPrmFilesIter) {
        ...
        if (strPrmFilesIter == strPrmFiles.begin()) {
            ...
            if (c_min.x < min.x) min.x = c_min.x;
            if (c_min.y < min.y) min.y = c_min.y;
            if (c_min.z < min.z) min.z = c_min.z;
            // change max extents
            if (c_max.x > max.x) max.x = c_max.x;
            if (c_max.y > max.y) max.y = c_max.y;
            if (c_max.z > max.z) max.z = c_max.z;
        }
    }
}
...
long RbtMOEGrid::WriteGrid(std::_Ios_Openmode mode) {
    ...
    if (ios_base::trunc == mode) ofstr.write(GRID_SIGNATURE.c_str(), GRID_SIGNATURE.size());  // signature
    ...
    for (vector<int>::iterator i_ext_iter = i_ext.begin(); i_ext_iter != i_ext.end(); ++i_ext_iter) {
    ...
    // write data vector
    for (RbtMOEGridData::iterator d_iter = myData.begin(); d_iter != myData.end(); ++d_iter) {
