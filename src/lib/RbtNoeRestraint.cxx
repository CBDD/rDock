...
RbtBool RbtNoeEndNames::isOK() const { return (!names.empty() && (type != Rbt::NOE_UNDEFINED)); }
...
ostream& operator<<(ostream& s, const RbtNoeEndNames& n) {
    if (n.names.size() == 1)
        ...
        s << "( ";
        for (RbtStringListConstIter iter = n.names.begin(); iter != n.names.end(); iter++) s << *iter << " ";
        s << ")";
    }
    return s;
}
...
ostream& operator<<(ostream& s, const RbtNoeEndAtoms& n) {
    if (n.atoms.size() == 1)
        ...
        if (n.atoms.size() <= 4) {
            ...
            s << n.atoms.front()->GetFullAtomName() << " ... " << n.atoms.back()->GetFullAtomName() << " <"
              << n.atoms.size() << " atoms> ";
        }
        s << ")";
    }
...
RbtBool RbtNoeRestraintAtoms::isSimple() const { return (from.atoms.size() == 1) && (to.atoms.size() == 1); }
...
RbtBool RbtStdRestraintAtoms::isSimple() const { return (from.atoms.size() == 1); }
