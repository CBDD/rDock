...
RbtModelPtr RbtPRMFactory::CreateReceptor() {
    ...
    if (m_pParamSource->isParameterPresent(_REC_FILE)) {
        if (m_iTrace > 0) {
            cout << endl
                 << "Using " << _REC_FILE << " as combined source of topology and 3D coordinates" << endl
                 << endl;
        }
        ...
        RbtBool isOK = theSource->isAtomListSupported() && theSource->isBondListSupported()
                       && theSource->isCoordinatesSupported();
        if (!isOK) {
            if (m_iTrace > 0) {
                cout << endl << "Incompatible file type for " << _REC_FILE << " option" << endl;
                cout << "File type must provide atom list, bond list and coordinates in a single file" << endl;
                cout << "Consider using " << _REC_TOPOL_FILE << " and " << _REC_COORD_FILE
                     << " to specify topology (atoms/bonds) and coordinate files separately" << endl
                     << endl;
            }
            throw RbtBadReceptorFile(_WHERE_, "Inappropriate molecular file type for " + _REC_FILE + " option");
        }
    ...
    // Read topology and coordinates separately from two molecular file sources
        ...
        if (m_iTrace > 0) {
            cout << endl << "Using " << _REC_TOPOL_FILE << " as topology source" << endl << endl;
        }
        RbtString strTopolFile = m_pParamSource->GetParameterValueAsString(_REC_TOPOL_FILE);
        RbtMolecularFileSourcePtr theTopolSource = CreateMolFileSource(strTopolFile);
        RbtBool isOK = theTopolSource->isAtomListSupported() && theTopolSource->isBondListSupported();
        if (!isOK) {
            if (m_iTrace > 0) {
                cout << endl << "Incompatible file type for " << _REC_TOPOL_FILE << " option" << endl;
                cout << "File type must provide topology information (atom list and bond list)" << endl << endl;
            }
            throw RbtBadReceptorFile(_WHERE_, "Inappropriate molecular file type for " + _REC_TOPOL_FILE + " option");
        }
        retVal = new RbtModel(theTopolSource);
        if (!bEnsemble) {
            // Now read coord file
            if (m_iTrace > 0) {
                cout << endl << "Using " << _REC_COORD_FILE << " as source of 3D coordinates" << endl << endl;
            }
            RbtString strCoordFile = m_pParamSource->GetParameterValueAsString(_REC_COORD_FILE);
            RbtMolecularFileSourcePtr theCoordSource = CreateMolFileSource(strCoordFile);
            isOK = theCoordSource->isAtomListSupported() && theCoordSource->isCoordinatesSupported();
            if (!isOK) {
                if (m_iTrace > 0) {
    ...
    if (bEnsemble) {
        ...
        if (m_iTrace > 0) {
            cout << endl
                 << "Using ensemble of " << _REC_COORD_FILE << "'s as source of 3D coordinates (N=" << n << ")" << endl
                 << endl;
        }
        for (RbtInt i = 1; i <= n; i++) {
            ...
            RbtMolecularFileSourcePtr theCoordSource = CreateMolFileSource(strCoordFile);
            RbtBool isOK = theCoordSource->isAtomListSupported() && theCoordSource->isCoordinatesSupported();
            if (!isOK) {
                if (m_iTrace > 0) {
                    cout << endl << "Incompatible file type for " << paramName << " option" << endl;
                    cout << "File type must provide coordinate information (atom list with 3D coords)" << endl << endl;
                }
                throw RbtBadReceptorFile(_WHERE_, "Inappropriate molecular file type for " + paramName + " option");
            }
            retVal->UpdateCoords(theCoordSource);
...
RbtModelList RbtPRMFactory::CreateSolvent() {
    ...
    if (m_pParamSource->isParameterPresent(_SOLV_FILE)) {
        ...
        RbtMolecularFileSourcePtr theSource = CreateMolFileSource(strFile);
        RbtBool isOK = theSource->isAtomListSupported() && theSource->isCoordinatesSupported();
        if (!isOK) {
            if (m_iTrace > 0) {
                cout << endl
                     << "Incompatible file type for " << _SOLV_SECTION << "::" << _SOLV_FILE << " option" << endl;
...
void RbtPRMFactory::AttachReceptorFlexData(RbtModel* pReceptor) {
    ...
    if (m_pParamSource->isParameterPresent(_REC_FLEX_DISTANCE)) {
        ...
        if (m_iTrace > 0) {
            cout << endl
                 << "Target OH/NH3 groups within " << flexDist << " A of docking site requested as flexible" << endl;
        }
        ...
        if (m_pParamSource->isParameterPresent(_REC_DIHEDRAL_STEP)) {
            RbtDouble dihedralStepSize = m_pParamSource->GetParameterValue(_REC_DIHEDRAL_STEP);
            pFlexData->SetParameter(RbtReceptorFlexData::_DIHEDRAL_STEP, dihedralStepSize);
            if (m_iTrace > 0) {
        ...
        if (m_iTrace > 0) {
            cout << endl << "RECEPTOR FLEXIBILITY PARAMETERS:" << endl << *pFlexData << endl;
        }
    }
}
...
void RbtPRMFactory::AttachLigandFlexData(RbtModel* pLigand) {
    ...
    if (m_iTrace > 0) {
        cout << endl << "LIGAND FLEXIBILITY PARAMETERS:" << endl << *pFlexData << endl;
    }
}
...
void RbtPRMFactory::AttachSolventFlexData(RbtModel* pSolvent) {
    ...
    pFlexData->SetParameter(RbtLigandFlexData::_TRANS_MODE, RbtChromElement::ModeToStr(transMode));
    pFlexData->SetParameter(RbtLigandFlexData::_ROT_MODE, RbtChromElement::ModeToStr(rotMode));
    ...
    if (m_iTrace > 0) {
        cout << endl << "SOLVENT FLEXIBILITY PARAMETERS:" << endl << *pFlexData << endl;
    }
}
...
RbtMolecularFileSourcePtr RbtPRMFactory::CreateMolFileSource(const RbtString& fileName) {
    ...
    if (fileTypeUpper == "MOL2") {
        ...
        RbtString strMassesFile = m_pParamSource->GetParameterValueAsString("RECEPTOR_MASSES_FILE");
    ...
    if (m_pParamSource->isParameterPresent("RECEPTOR_SEGMENT_NAME")) {
        RbtString strSegmentName = m_pParamSource->GetParameterValueAsString("RECEPTOR_SEGMENT_NAME");
    ...
    if (nAtoms == 0) {
        ...
        throw RbtBadReceptorFile(_WHERE_, "Zero atoms provided by " + retVal->GetFileName());
    } else {
        if (m_iTrace > 0) {
