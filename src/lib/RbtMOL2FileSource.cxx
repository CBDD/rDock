...
RbtMOL2FileSource::RbtMOL2FileSource(const RbtString& fileName, RbtBool bImplHydrogens):
    ...
    m_spParamSource =
        RbtParameterFileSourcePtr(new RbtParameterFileSource(Rbt::GetRbtFileName("data/sf", "RbtIonicAtoms.prm")));
...
RbtMOL2FileSource::~RbtMOL2FileSource() { _RBTOBJECTCOUNTER_DESTR_("RbtMOL2FileSource"); }
...
void RbtMOL2FileSource::Parse(void) {
    ...
    if (!m_bParsedOK) {
        ...
        try {
            ...
            for (RbtFileRecListIter lineIter = m_lineRecs.begin(); lineIter < m_lineRecs.end(); ++lineIter) {
                ...
                if (ssIter != m_ssInfo.end()) {
                    ...
                    for (RbtAtomListIter aIter = ssAtomList.begin(); aIter != ssAtomList.end(); aIter++) {
                        ...
                        if ((sID != atom_sID) || (sName != atom_sName)) {
                            cout << _CT << ": WARNING Mismatch between SUBSTRUCTURE (" << sName << sID
                                 << ") and ATOM (" << atom_sName << atom_sID << ") info" << endl;
                        }
                        ...
                        // cout << "Updating segment name to " << chain << " for " << (*aIter)->GetFullAtomName() <<
                        // endl;
                    }
                } else {
            ...
            if (m_ssInfo.size() != nSubstructures) {
                cout << _CT << ": WARNING Mismatch in #SUBSTRUCTURES read: Expected=" << nSubstructures
                     << "; Actual = " << m_ssInfo.size() << endl;
            }
...
void RbtMOL2FileSource::ParseRecordATOM(const RbtString& aLine) {
    ...
    if (tokens.size() < 6) throw RbtFileParseError(_WHERE_, "Corrupted MOL2 file: not enough fields in ATOM ");
...
void RbtMOL2FileSource::ParseRecordBOND(const RbtString& aLine) {
    ...
    if ((origin_bond_id < 1) || (origin_bond_id > m_atomList.size()) || (target_bond_id < 1)
        || (target_bond_id > m_atomList.size()))
...
void RbtMOL2FileSource::ParseRecordSUBSTRUCTURE(const RbtString& aLine) {
    ...
    if (tokens.size() < 3) throw RbtFileParseError(_WHERE_, "Corrupted MOL2 file: not enough fields in SUBSTRUCTURE");
...
void RbtMOL2FileSource::ParseCountFields(const RbtString& aLine) {
    RbtStringList tokens;
    Tokenize(aLine, tokens);
    if (tokens.size() < 1) throw RbtFileParseError(_WHERE_, "Corrupted MOL2: no atom numbers in MOLECULE record.");
    // number of atoms:
    ...
    if (tokens.size() > 3) nFeatures = atoi(tokens[3].c_str());
...
void RbtMOL2FileSource::FixImplicitHydrogenCount() {
    ...
    for (RbtAtomListIter iter = cList.begin(); iter != cList.end(); iter++) {
        ...
        RbtInt expected = (hyb == RbtAtom::SP3)    ? 4
                          : (hyb == RbtAtom::SP2)  ? 3
                          : (hyb == RbtAtom::AROM) ? 3
                          : (hyb == RbtAtom::SP)   ? 2
                                                   : 4;
        ...
        if ((hyb == RbtAtom::AROM) && (nImplH > 0)) {
            ...
            if ((nImplH + nExplH) > 1) {
                cout << _CT << ": INFO Too few bonds detected to aromatic carbon " << (*iter)->GetFullAtomName()
                     << "; #hydrogens capped at 1" << endl;
                nImplH = 1 - nExplH;
            }
        }
        if (nImplH > 0) {
...
void RbtMOL2FileSource::Tokenize(const RbtString& aString, RbtStringList& aTokensBuf) {
    ...
    while (ss >> buf) aTokensBuf.push_back(buf);
}
