...
void RbtChromFactory::VisitReceptorFlexData(RbtReceptorFlexData* pFlexData) {
    ...
    if (pModel && pDockSite) {
        RbtDouble flexDistance = pFlexData->GetParameter(RbtReceptorFlexData::_FLEX_DISTANCE);
        RbtDouble dihedralStepSize = pFlexData->GetParameter(RbtReceptorFlexData::_DIHEDRAL_STEP);
        ...
        for (RbtBondListConstIter iter = rotBondList.begin(); iter != rotBondList.end(); ++iter) {
            if (isOH(*iter) || isNH3(*iter)) {
                ...
                if (bIsInRange(pAtom1) && bIsInRange(pAtom2)) {
                    m_pChrom->Add(new RbtChromDihedralElement(*iter, noTetheredAtoms, dihedralStepSize));
        ...
        if (!modelMutatorBondList.empty()) {
            m_spMutator = RbtModelMutatorPtr(new RbtModelMutator(pModel, modelMutatorBondList, noTetheredAtoms));
        } else {
            m_spMutator.SetNull();
        }
    }
...
void RbtChromFactory::VisitLigandFlexData(RbtLigandFlexData* pFlexData) {
    ...
    RbtDockingSite* pDockSite = pFlexData->GetDockingSite();
    if (pModel && pDockSite) {
        RbtDouble transStepSize = pFlexData->GetParameter(RbtLigandFlexData::_TRANS_STEP);
        RbtDouble rotStepSize = pFlexData->GetParameter(RbtLigandFlexData::_ROT_STEP);
        RbtDouble dihedralStepSize = pFlexData->GetParameter(RbtLigandFlexData::_DIHEDRAL_STEP);
        ...
        RbtString dihedralModeStr = pFlexData->GetParameter(RbtLigandFlexData::_DIHEDRAL_MODE);
        ...
        RbtDouble maxDihedral = pFlexData->GetParameter(RbtLigandFlexData::_MAX_DIHEDRAL);
        ...
        if ((transMode == RbtChromElement::TETHERED) && (transStepSize > maxTrans)) {
        ...
        if ((dihedralMode == RbtChromElement::TETHERED) && (dihedralStepSize > maxDihedral)) {
        ...
        if (!rotBondList.empty()) {
            m_spMutator = RbtModelMutatorPtr(new RbtModelMutator(pModel, rotBondList, tetheredAtoms));
        } else {
            m_spMutator.SetNull();
        }
    }
...
void RbtChromFactory::VisitSolventFlexData(RbtSolventFlexData* pFlexData) {
    ...
    RbtDockingSite* pDockSite = pFlexData->GetDockingSite();
    if (pModel && pDockSite) {
        RbtDouble occupancyProb = pFlexData->GetParameter(RbtSolventFlexData::_OCCUPANCY);
        RbtDouble stepSize = pFlexData->GetParameter(RbtSolventFlexData::_OCCUPANCY_STEP);
