...
RbtVdwGridSF::RbtVdwGridSF(const RbtString& strName): RbtBaseSF(_CT, strName), m_bSmoothed(true) {
    // Add parameters
...
void RbtVdwGridSF::SetupScore() {
    ...
    if (iTrace > 1) {
        if (bHasUndefined) {
            ...
            cout << "There is no grid for the UNDEFINED atom type. An error will be thrown if a grid is missing for "
                    "any atom type"
                 << endl;
        }
    }
    ...
    for (RbtAtomRListConstIter iter = m_ligAtomList.begin(); iter != m_ligAtomList.end(); iter++) {
        ...
        if (m_grids[aType].Null()) {
            RbtString strError = "No vdw grid available for " + (*iter)->GetFullAtomName() + " (type "
