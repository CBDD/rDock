...
RbtChromOccupancyElement::RbtChromOccupancyElement(RbtModel* pModel, RbtDouble stepSize, RbtDouble threshold):
    m_value(1.0) {
...
RbtChromOccupancyElement::~RbtChromOccupancyElement() { _RBTOBJECTCOUNTER_DESTR_(_CT); }

void RbtChromOccupancyElement::Reset() { m_value = m_spRefData->GetInitialValue(); }

void RbtChromOccupancyElement::Randomise() { m_value = GetRand().GetRandom01(); }
...
void RbtChromOccupancyElement::SyncFromModel() { m_value = m_spRefData->GetModelValue(); }

void RbtChromOccupancyElement::SyncToModel() { m_spRefData->SetModelValue(m_value); }

RbtChromElement* RbtChromOccupancyElement::clone() const { return new RbtChromOccupancyElement(m_spRefData, m_value); }

void RbtChromOccupancyElement::GetVector(RbtDoubleList& v) const { v.push_back(m_value); }
...
void RbtChromOccupancyElement::GetStepVector(RbtDoubleList& v) const { v.push_back(m_spRefData->GetStepSize()); }
...
void RbtChromOccupancyElement::Print(ostream& s) const { s << "OCCUPANCY " << m_value << endl; }
