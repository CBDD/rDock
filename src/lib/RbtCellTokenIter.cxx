...
// Constructors
///////////////////
RbtCellTokenIter::RbtCellTokenIter(const RbtGPChromosomePtr c, RbtContextPtr contextp): chrom(c) {
    ...
    if (chrom->IsProgramInput(startCell)) {
        ...
        if (comm.IsRCte()) {
            ...
            for (RbtInt i = nargs - 1; i >= 0; i--) cells.push(chrom->GetArgument(startCell, i));
        }
    }
    ...
    if (cells.size() > 0) {
        ...
        if (chrom->IsProgramInput(cell)) {
            ...
            if (comm.IsRCte()) {
                ...
                current = new RbtToken(comm);
                RbtInt nargs = comm.GetNArgs();
                for (RbtInt i = nargs - 1; i >= 0; i--) cells.push(chrom->GetArgument(cell, i));
            }
        }
    }
}
