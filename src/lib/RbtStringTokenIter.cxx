...
// Constructors
///////////////////
RbtStringTokenIter::RbtStringTokenIter(const istreamPtr fn, RbtContextPtr co): filep(fn), contextp(co) {
    (istream&)(*filep) >> strtok;
    _RBTOBJECTCOUNTER_CONSTR_(_CT);
}

RbtStringTokenIter::RbtStringTokenIter(const RbtStringTokenIter& ti):
    current(ti.current), filep(ti.filep), strtok(ti.strtok), contextp(ti.contextp) {
...
RbtTokenPtr RbtStringTokenIter::translate(RbtString s) {
    if (s.length() == 0) throw RbtError(_WHERE_, "Missing token, can't translate the expression");
    if (s == "+") return new RbtToken(RbtCommands(RbtCommands::ADD));
    if (s == "-") return new RbtToken(RbtCommands(RbtCommands::SUB));
    if (s == "*") return new RbtToken(RbtCommands(RbtCommands::MUL));
    if (s == "/") return new RbtToken(RbtCommands(RbtCommands::DIV));
    if (s == "if") {
        return new RbtToken(RbtCommands(RbtCommands::IF));
    }
    if (s == "log") return new RbtToken(RbtCommands(RbtCommands::LOG));
    if (s == "exp") return new RbtToken(RbtCommands(RbtCommands::EXP));
    if (s == "and") return new RbtToken(RbtCommands(RbtCommands::AND));
    if ((string::npos != s.find("SCORE")) || (string::npos != s.find("SITE")) || (string::npos != s.find("LIG"))) {
