...
RbtChrom::RbtChrom(const RbtModelList& modelList): RbtChromElement(), m_modelList(modelList) {
    for (RbtModelListConstIter iter = m_modelList.begin(); iter != m_modelList.end(); ++iter) {
        if ((*iter).Ptr()) {
            Add((*iter)->GetChrom());
        }
    }
...
RbtChrom::~RbtChrom() {
    for (RbtChromElementListIter iter = m_elementList.begin(); iter != m_elementList.end(); ++iter) {
        delete *iter;
    }
...
void RbtChrom::SyncToModel() {
    ...
    for (RbtModelListIter iter = m_modelList.begin(); iter != m_modelList.end(); ++iter) {
