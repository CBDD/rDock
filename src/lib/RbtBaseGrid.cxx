...
RbtBaseGrid::RbtBaseGrid(const RbtCoord& gridMin, const RbtCoord& gridStep, RbtUInt NX, RbtUInt NY, RbtUInt NZ,
                         RbtUInt NPad):
    m_step(gridStep), m_NX(NX), m_NY(NY), m_NZ(NZ), m_N(NX * NY * NZ), m_SX(NY * NZ), m_SY(NZ), m_SZ(1), m_NPad(NPad) {
...
// Default destructor
RbtBaseGrid::~RbtBaseGrid() { _RBTOBJECTCOUNTER_DESTR_("RbtBaseGrid"); }
        ...
        if (rX2 <= rad2) {
            ...
            for (iY = iMinY, rY = GetYCoord(iMinY) - c.y; iY <= iMaxY; iY++, rY += m_step.y) {
                ...
                if (rXY2 <= rad2) {
                    ...
                    RbtUInt iZ;
                    RbtUInt iXYZ;
                    for (iZ = iMinZ, iXYZ = GetIXYZ(iX, iY, iMinZ), rZ = GetZCoord(iMinZ) - c.z; iZ <= iMaxZ;
                         iZ++, iXYZ++, rZ += m_step.z) {
