...
RbtTransformAgg::~RbtTransformAgg() {
    ...
    DEBUG_ERR(_CT << "::~" << _CT << "(): Deleting child transforms of " << GetName() << endl);
    ...
    while (m_transforms.size() > 0) {
        ...
        DEBUG_ERR("Deleting " << pTransform->GetName() << " from " << GetName() << endl);
        delete pTransform;
    }
...
void RbtTransformAgg::Add(RbtBaseTransform* pTransform) {
    ...
    DEBUG_ERR(_CT << "::Add(): Adding " << pTransform->GetName() << " to " << GetName() << endl);
...
void RbtTransformAgg::Remove(RbtBaseTransform* pTransform) {
    ...
    if (iter == m_transforms.end()) {
        ...
        // Assertion: parent of child is this object
        Assert<RbtAssert>(!TRANSFORMAGG_CHECK || pTransform->m_parent == this);
        DEBUG_ERR(_CT << "::Remove(): Removing " << pTransform->GetName() << " from " << GetName() << endl);
...
void RbtTransformAgg::Register(RbtWorkSpace* pWorkSpace) {
    DEBUG_ERR(_CT << "::Register(): Registering child transforms of " << GetName() << endl);
...
void RbtTransformAgg::Unregister() {
    DEBUG_ERR(_CT << "::Unregister(): Unregistering child transforms of " << GetName() << endl);
    for (RbtBaseTransformListIter iter = m_transforms.begin(); iter != m_transforms.end(); iter++) {
        (*iter)->Unregister();
    }
}
