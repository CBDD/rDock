                ...
                if (Rbt::FindAtom(bondedAtomList, Rbt::isPiAtom()) != bondedAtomList.end()) {
                    ...
                    if (bondedAtomList.size() == 3) {
                        ...
                        if (dist > 0.1) {  // Not planar
                            DEBUG_OUT("Not planar" << endl);
                            break;
                        }
                    }
                    (*iter)->SetHybridState(RbtAtom::TRI);
                    DEBUG_OUT("Changing " << (*iter)->GetAtomName() << " from SP3 to TRI" << endl);
                }
                break;
            default:
                break;
...
void RbtMdlFileSource::SetupPosIonisableGroups() {
    ...
    for (RbtAtomListIter iter = nsp3AtomList.begin(); iter != nsp3AtomList.end(); iter++) {
        ...
        if (Rbt::FindAtom(bondedAtomList, Rbt::isPiAtom()) == bondedAtomList.end()) {
            ...
            DEBUG_OUT("Amine adjacent to pi-atom, not protonated: " << (*iter)->GetAtomName() << endl);
        }
    }
                ...
                switch (Rbt::GetNumAtoms(bondedAtomList, bIsN_TRI)) {
                    ...
                    case 1:
                        ...
                        if ((Rbt::GetNumAtoms(nsp2BondedAtomList, bIsC_SP2) == 2)
                            ...
                            DEBUG_OUT("Possible imidazole, bonding requirements for N_SP2 and N_TRI met" << endl);
                            ...
                            if (atoms12Conn.size() == 1) {
                                DEBUG_OUT("5-membered imidazole ring found" << endl);
                                ...
                                if (atoms12Conn.size() == 1) {  // Imidazole!!
                                    ...
                                    DEBUG_OUT("Fused imidazole - ignored " << (*nsp2Iter)->GetAtomName() << endl);
                                }
                            } else {
                                DEBUG_OUT("Not a 5-membered ring" << endl);
                            }
                        }
                        // AMIDINE Check that the N_SP2 is bonded to 1 hydrogen and the N_TRI is bonded to 2 hydrogens
                            ...
                            DEBUG_OUT("Bonding requirements for N_SP2 and N_TRI not met" << endl);
                        }
                        break;
                        //////////////////////
                        // End of case #N_TRI=1
...
void RbtMdlFileSource::SetupNSP2Plus() {
    ...
    for (RbtAtomListIter iter = nsp2PlusAtomList.begin(); iter != nsp2PlusAtomList.end(); iter++) {
        ...
        DEBUG_OUT(csp2AtomList.size() << " CSP2 atoms found bonded to NSP2+" << endl);
        ...
        DEBUG_OUT(csp2AtomList.size() << " are non-bridgehead" << endl);
        ...
        DEBUG_OUT("Checking N_SP2+ atom " << (*iter)->GetAtomName() << endl);
        if (guanIter != csp2AtomList.end())
            DEBUG_OUT((*guanIter)->GetAtomName() << " is a guanidinium-like carbon" << endl);
        if (imidIter != csp2AtomList.end())
            DEBUG_OUT((*imidIter)->GetAtomName() << " is a imidazole-like carbon" << endl);
        ...
        RbtAtomList hbdAtomList = Rbt::GetAtomList(Rbt::GetBondedAtomList(*iter), bIsH);
        DEBUG_OUT("Found " << hbdAtomList.size() << " hydrogens bonded to " << (*iter)->GetAtomName() << endl);
        ...
        if (csp2Iter != csp2AtomList.end()) {
            ...
            for (RbtAtomListConstIter nIter = ntriAtomList.begin(); nIter != ntriAtomList.end(); nIter++) {
                ...
                DEBUG_OUT("Found " << hAtomList.size() << " hydrogens bonded to " << (*nIter)->GetAtomName() << endl);
...
void RbtMdlFileSource::SetupSegmentNames() {
    ...
    for (nSeg = 1, seed = m_atomList.begin(); seed != m_atomList.end();
        ...
        DEBUG_OUT("Segment " << strSegName << " contains " << nSize << " atoms" << endl);
    ...
    DEBUG_OUT("Largest segment is " << strLargestSegName << endl);
...
void RbtMdlFileSource::RemoveNonPolarHydrogens() {
    ...
    for (auto bond: m_bondList) {
        DEBUG_OUT(
            "Bond ID=" << bond.GetBondId() << " (" << bond.GetAtom1Ptr()->GetAtomName() << "-"
                       << bond.GetAtom2Ptr()->GetAtomName() << ")" << endl
        );
    }
}
