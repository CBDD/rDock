...
#ifdef _DEBUG
                        cout << "Distance from " << (*iter)->GetAtomName() << " to plane of substituents=" << dist
                             << endl;
#endif                                     //_DEBUG
...
#ifdef _DEBUG
                        cout << "Adding " << nMinVal - nValency << " implicit hydrogens to " << (*iter)->GetAtomName()
                             << endl;
#endif  //_DEBUG
                    }
                    break;
...
void RbtMdlFileSource::RemoveNonPolarHydrogens() {
    ...
    for (RbtBondListConstIter bIter = m_bondList.begin(); bIter != m_bondList.end(); bIter++) {
        cout << "Bond ID=" << (*bIter)->GetBondId() << " (" << (*bIter)->GetAtom1Ptr()->GetAtomName() << "-"
