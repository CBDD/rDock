...
RbtSAIdxSF::RbtSAIdxSF(const RbtString& aName):
    ...
    m_spSolvSource =
        RbtParameterFileSourcePtr(new RbtParameterFileSource(Rbt::GetRbtFileName("data/sf", "solvation_asp.prm")));
    Setup();
...
void RbtSAIdxSF::SetupReceptor() {
    ...
    for (HHS_SolvationRListConstIter iter = theRSPList.begin(); iter != theRSPList.end(); iter++) {
        ...
        if (isNearCavity(pAtom)) {
            ...
            if (iTrace > 1) {
                cout << _CT << ": Peripheral rigid atom within range of flexible atoms: " << pAtom->GetFullAtomName()
                     << endl;
            }
        }
    }
    ...
    if (iTrace > 1) {
        ...
        for (HHS_SolvationRListConstIter iter = theCavList.begin(); iter != theCavList.end(); iter++) {
            ...
            cout << _CT << ": " << pAtom->GetFullAtomName() << ", " << hhsType.Type2Str(t) << ", " << sasa << ", "
                 << asp << ", " << energy << endl;
        }
        if (m_bFlexRec) {
...
RbtDouble RbtSAIdxSF::RawScore(void) const {
    ...
    for (HHS_SolvationRListConstIter iter = theLSPList.begin(); iter != theLSPList.end(); ++iter) (*iter)->Restore();
    for (HHS_SolvationRListConstIter iter = theCavList.begin(); iter != theCavList.end(); ++iter) (*iter)->Restore();
    ...
    RbtDouble theScore = (m_lig_bound - m_lig_0) + (m_site_bound - m_site_0) + (m_solvent_bound - m_solvent_0);
    return theScore;
}

RbtDouble RbtSAIdxSF::GetP_i(RbtHHSType::eType theType) const { return m_solvTable[theType].p; }

RbtDouble RbtSAIdxSF::GetR_i(RbtHHSType::eType theType) const { return m_solvTable[theType].r; }
...
void RbtSAIdxSF::ScoreMap(RbtStringVariantMap& scoreMap) const {
    if (isEnabled()) {
        ...
        RbtDouble inter_rs =
            (m_site_bound - m_site_free) + (m_lig_bound - m_lig_free) + (m_solvent_bound - m_solvent_free);
...
void RbtSAIdxSF::HandleRequest(RbtRequestPtr spRequest) {
    ...
    switch (spRequest->GetID()) {
        ...
        case ID_REQ_SF_PARTITION:
            if (params.size() == 1) {
                if (iTrace > 2) {
                    cout << _CT << "::HandleRequest: Partitioning " << GetFullName() << " at distance=" << params[0]
                         << endl;
                }
                Partition(theLSPList, params[0]);
            } else if ((params.size() == 2) && (params[0].String() == GetFullName())) {
                if (iTrace > 2) {
                    cout << _CT << "::HandleRequest: Partitioning " << GetFullName() << " at distance=" << params[1]
                         << endl;
                }
    ...
    for (RbtAtomListConstIter iter = atomList.begin(); iter != atomList.end(); iter++) {
        ...
        if (t != RbtHHSType::H) {
            ...
            if (t == RbtHHSType::UNDEFINED)
                cout << _CT << ": WARNING - CANNOT ASSIGN SOLVATION ATOM TYPE FOR " << (*iter)->GetFullAtomName()
                     << endl;
        } else if (iTrace > 1)
