...
RbtFilter::RbtFilter(RbtString strfilter, RbtBool filter): RbtBaseObject(_CT, "Filter") {
    ...
    for (RbtInt i = 0; i < nTermFilters; i++) {
        ...
        SmartPtr<istream> istrp(new istringstream(s.c_str()));
...
void RbtFilter::SetupReceptor() {
    // read filter file
    ((RbtStringContextPtr)contextp)->UpdateSite(m_spReceptor, GetWorkSpace()->GetDockingSite());
}
...
void RbtFilter::SetupScore() {
    ((RbtStringContextPtr)contextp)->UpdateScores(GetWorkSpace()->GetSF(), GetWorkSpace()->GetModel(1));
