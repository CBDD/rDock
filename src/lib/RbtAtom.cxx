...
ostream& RbtAtom::Print(ostream& s) const {
    ...
    if (m_pModel != NULL) strModelName = m_pModel->GetName();

    RbtTriposAtomType triposType;

    return s << "(" << strModelName << ")"
             << ", ID=" << m_nAtomId << ", AtNo=" << m_nAtomicNo << ", " << m_strSegmentName << "," << m_strSubunitName
             << "," << m_strSubunitId << "," << m_strAtomName
             << ", Hybrid=" << Rbt::ConvertHybridStateToString(m_eState) << ", #Hyd=" << m_nHydrogens
             << ", GrpChg=" << m_dGroupCharge << ", Tripos="
...
RbtBool RbtAtom::GetEnabled() const { return (m_pModel) ? m_pModel->GetEnabled() : true; }
...
RbtUInt RbtAtom::GetNumCyclicBonds() const {
    ...
    for (RbtBondMapConstIter iter = m_bondMap.begin(); iter != m_bondMap.end(); iter++)
        if ((*iter).first->GetCyclicFlag()) nCount++;
    return nCount;
}
...
RbtBondMap RbtAtom::GetCyclicBondMap() const {
    ...
    for (RbtBondMapConstIter iter = m_bondMap.begin(); iter != m_bondMap.end(); iter++)
        if ((*iter).first->GetCyclicFlag()) cyclicBondMap.insert(*iter);
...
RbtUInt RbtAtom::GetCoordinationNumber() const { return m_bondMap.size() + m_nHydrogens; }
...
RbtBool Rbt::isPiAtom::operator()(const RbtAtom* pAtom) const {
    const RbtAtom::eHybridState hybrid = pAtom->GetHybridState();
    return ((hybrid == RbtAtom::SP2) || (hybrid == RbtAtom::AROM) || (hybrid == RbtAtom::TRI));
}
...
RbtBool Rbt::isAtomNearCoordList::operator()(const RbtAtom* pAtom) const {
    ...
    for (RbtCoordListConstIter iter = cl.begin(); iter != cl.end(); iter++) {
        if (Rbt::Length2(atomCoord - (*iter)) <= r2) return true;
    }
    return false;
}
...
RbtBool Rbt::isAtom_12Connected::operator()(RbtAtom* pAtom2) const {
    ...
    return std::find_if(bondedAtomList1.begin(), bondedAtomList1.end(), std::bind2nd(Rbt::isAtomPtr_eq(), pAtom2))
           != bondedAtomList1.end();
}
    ...
    switch (nAtomicNo) {
        ...
        case 16:  // S
            ...
            for (iter = bondedAtomList.begin(); iter != bondedAtomList.end(); iter++) {
                ...
                if (isO(*iter) && isSP2(*iter)) return false;
            }
            return true;
        case 9:  // F
            return false;
    ...
    for (RbtAtomListConstIter iter = atomList.begin(); iter != atomList.end(); iter++) {
        ...
        switch (nFormat) {
            case 0:
                ...
                break;
            default:
                s << "pick " << spAtom->GetAtomName() << " .and. segm " << spAtom->GetSegmentName() << " = col "
                  << nColor << endl;
        }
    }
}
...
void Rbt::RemoveZwitterions(RbtAtomList& atomList) {
    ...
    for (RbtAtomListConstIter aIter = anionicAtomList.begin(); aIter != anionicAtomList.end(); aIter++) {
        ...
        c14begin = std::stable_partition(c14begin, cationicAtomList.end(), Rbt::isAtom_12Connected(*aIter));
        c14begin = std::stable_partition(c14begin, cationicAtomList.end(), Rbt::isAtom_13Connected(*aIter));
    }
    ...
    for (RbtAtomListConstIter cIter = cationicAtomList.begin(); cIter != cationicAtomList.end(); cIter++) {
        // Need to partition in two stages: firstly 1-2 connected, then 1-3 connected
        a14begin = std::stable_partition(a14begin, anionicAtomList.end(), Rbt::isAtom_12Connected(*cIter));
        a14begin = std::stable_partition(a14begin, anionicAtomList.end(), Rbt::isAtom_13Connected(*cIter));
    }
