...
RbtChromDihedralRefData::RbtChromDihedralRefData(RbtBondPtr spBond, RbtAtomList tetheredAtoms, RbtDouble stepSize,
...
RbtChromDihedralRefData::~RbtChromDihedralRefData() { _RBTOBJECTCOUNTER_DESTR_(_CT); }
...
void RbtChromDihedralRefData::SetModelValue(RbtDouble dihedralAngle) {
    ...
    if (fabs(delta) > 0.001) {
        ...
        for (RbtAtomRListIter iter = m_rotAtoms.begin(); iter != m_rotAtoms.end(); ++iter) {
...
void RbtChromDihedralRefData::Setup(RbtBondPtr spBond, const RbtAtomList& tetheredAtoms) {
    ...
    RbtAtomList bondedAtoms2 =
        Rbt::GetAtomList(Rbt::GetBondedAtomList(pAtom2), std::not1(std::bind2nd(Rbt::isAtomPtr_eq(), pAtom3)));
    RbtAtomList bondedAtoms3 =
        Rbt::GetAtomList(Rbt::GetBondedAtomList(pAtom3), std::not1(std::bind2nd(Rbt::isAtomPtr_eq(), pAtom2)));
    ...
    RbtInt nSelected = (nTethered == 0) ? Rbt::GetNumSelectedAtoms(atomList) : Rbt::GetNumSelectedAtoms(tetheredAtoms);
    ...
    m_rotAtoms.clear();
    std::copy_if(atomList.begin(), atomList.end(), std::back_inserter(m_rotAtoms), Rbt::isAtomSelected());
}
