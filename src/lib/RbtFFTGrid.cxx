...
RbtFFTGrid::RbtFFTGrid(const RbtRealGrid& grid): RbtRealGrid(grid) { _RBTOBJECTCOUNTER_COPYCONSTR_("RbtFFTGrid"); }

RbtFFTGrid::RbtFFTGrid(const RbtBaseGrid& grid): RbtRealGrid(grid) { _RBTOBJECTCOUNTER_COPYCONSTR_("RbtFFTGrid"); }
    ...
    while (!stillToProcess.empty()) {
        ...
        if (currentPeak.size() >= minVol) {
            ...
            cout << "Finished growing peak #" << peakMap.size() << ", pos=" << spPeak->coord
                 << ", height=" << spPeak->height << ", volume=" << spPeak->volume << endl
                 << endl;
#endif  //_DEBUG
        }
    }
...
void RbtFFTGrid::OwnPrint(ostream& ostr) const { ostr << "Class\t" << _CT << endl; }
