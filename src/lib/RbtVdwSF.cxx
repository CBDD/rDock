...
RbtVdwSF::RbtVdwSF(): m_use_4_8(true), m_use_tripos(false), m_rmax(1.5), m_ecut(1.0), m_e0(1.5) {
    ...
    m_spVdwSource =
        RbtParameterFileSourcePtr(new RbtParameterFileSource(Rbt::GetRbtFileName("data/sf", "Tripos52_vdw.prm")));
    Setup();
...
//  return score;
//}
RbtDouble RbtVdwSF::MaxVdwRange(const RbtAtom* pAtom) const { return m_maxRange[pAtom->GetTriposType()]; }

RbtDouble RbtVdwSF::MaxVdwRange(RbtTriposAtomType::eType t) const { return m_maxRange[t]; }
        ...
        for (RbtInt j = i; j < RbtTriposAtomType::MAXTYPES; j++) {
            ...
            if (iTrace > 3) {
                cout << triposType.Type2Str(RbtTriposAtomType::eType(i)) << ","
                     << triposType.Type2Str(RbtTriposAtomType::eType(j)) << "," << prms.A << "," << prms.B << ","
                     << prms.rmin << "," << prms.kij << "," << sqrt(prms.rmax_sq) << endl;
            }
        }
    }
        ...
        for (RbtVdwRowIter iter2 = (*iter1).begin(); iter2 != (*iter1).end(); iter2++) {
            ...
            if (iTrace > 3) {
                cout << triposType.Type2Str(RbtTriposAtomType::eType(iter1 - m_vdwTable.begin())) << ","
                     ...
                     << sqrt((*iter2).rcutoff_sq) << "," << (*iter2).ecutoff << "," << (*iter2).slope << ","
                     << (*iter2).e0 << endl;
            }
        }
    }
...
void RbtVdwSF::BuildIntraMap(const RbtAtomRList& atomList1, const RbtAtomRList& atomList2, RbtAtomRListList& intns)
    ...
    for (RbtAtomRListConstIter iter = atomList1.begin(); iter != atomList1.end(); iter++) {
        ...
        if (GetTrace() > 2) {
            ...
            if (!intns[id].empty()) {
                cout << "\t" << intns[id].front()->GetFullAtomName() << " to " << intns[id].back()->GetFullAtomName();
            }
...
void RbtVdwSF::Partition(
    ...
    for (RbtAtomRListListIter iter = prtIntns.begin(); iter != prtIntns.end(); iter++) (*iter).clear();
    if (iTrace > 3) {
