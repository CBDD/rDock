...
RbtSFAgg::~RbtSFAgg() {
    ...
    DEBUG(_CT << "::~" << _CT << "(): Deleting child scoring functions of " << GetName() << endl);
    ...
    while (m_sf.size() > 0) {
        ...
        DEBUG("Deleting " << pSF->GetName() << " from " << GetName() << endl);
        delete pSF;
    }
...
void RbtSFAgg::Add(RbtBaseSF* pSF) {
    ...
    pSF->Orphan();
    pSF->m_parent = this;
    DEBUG(_CT << "::Add(): Adding " << pSF->GetName() << " to " << GetName() << endl);
    m_sf.push_back(pSF);
}
...
void RbtSFAgg::Remove(RbtBaseSF* pSF) {
    ...
    if (iter == m_sf.end()) {
        ...
        // Assertion: parent of child is this object
        Assert<RbtAssert>(!SFAGG_CHECK || pSF->m_parent == this);
        DEBUG(_CT << "::Remove(): Removing " << pSF->GetName() << " from " << GetName() << endl);
        m_sf.erase(iter);
...
void RbtSFAgg::Register(RbtWorkSpace* pWorkSpace) {
    ...
    DEBUG(_CT << "::Register(): Registering child scoring functions of " << GetName() << endl);
...
void RbtSFAgg::Unregister() {
    DEBUG(_CT << "::Unregister(): Unregistering child scoring functions of " << GetName() << endl);
    for (RbtBaseSFListIter iter = m_sf.begin(); iter != m_sf.end(); iter++) {
        (*iter)->Unregister();
    }
