...
RbtChromElement::eMode RbtChromElement::StrToMode(const RbtString& modeStr) {
    eMode retVal;
    ...
    if (modeStrUpper == "FIXED") {
        ...
        throw RbtBadArgument(_WHERE_, "Unknown chromosome sampling mode (" + modeStr + ")");
    }
    return retVal;
}
...
RbtChromElement::RbtChromElement(): m_rand(Rbt::GetRbtRand()) { _RBTOBJECTCOUNTER_CONSTR_(_CT); }
...
bool operator!=(const RbtChromElement& c1, const RbtChromElement& c2) { return !(c1 == c2); }
...
void Rbt::Crossover(
    ...
    RbtInt ixend =
        (ixbegin == 0) ? rand.GetRandomInt(length1 - 1) + 1 : rand.GetRandomInt(length1 - ixbegin) + ixbegin + 1;
