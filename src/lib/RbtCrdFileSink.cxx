...
RbtCrdFileSink::~RbtCrdFileSink() { _RBTOBJECTCOUNTER_DESTR_("RbtCrdFileSink"); }
...
void RbtCrdFileSink::Render() {
    ...
    try {
        ...
        for (RbtAtomListConstIter aIter = atomList.begin(); aIter != atomList.end(); aIter++) {
            ...
            if (GetMultiConf()) ostr << setw(5) << m_nAtomId;
            ...
            ostr << setw(4) << spAtom->GetSubunitName().c_str() << setw(0) << " " << setw(4)
                 << spAtom->GetAtomName().c_str();
            ostr.setf(ios_base::right, ios_base::adjustfield);
            ostr << setw(10) << spAtom->GetX() << setw(10) << spAtom->GetY() << setw(10) << spAtom->GetZ() << setw(0)
                 << " ";

            // DM 21 Apr 1999 - if we are not using the model segment names, use the incremental segment ID from the
            // sink DM 11 Feb 1999 - output leading zeros for segID, so they sort correctly when read as text
