...
// Default destructor
RbtModel::~RbtModel() {
    DEBUG_ERR("~RbtModel: deleting " << m_strName << " (" << m_atomList.size() << " atoms, " << m_bondList.size()
         << " bonds)" << endl);
...
const RbtAtomRList& RbtModel::GetFlexIntns(RbtAtom* pAtom) const {
    if (isFlexible()) {
        ...
        Assert<RbtAssert>(!MUT_CHECK || (id >= 0 && (RbtUInt)id < flexIntns.size()));
