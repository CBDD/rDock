...
RbtSetupPMFSF::RbtSetupPMFSF(const RbtString& strName): RbtBaseSF(_CT, strName) {
...
RbtPMFType RbtSetupPMFSF::GetPMFfor_rC(RbtAtomPtr anAtom) {
    ...
    cout << _CT << " " << anAtom->GetFullAtomName() << " Hybrid : " << Rbt::ConvertHybridStateToString(theHybState)
         << endl;
#endif  //_DEBUG1
...
#endif  //_DEBUG1
    // check charge
    if (anAtom->GetGroupCharge() < 0.0) return OC;
...
#endif  //_DEBUG1
    // check for charge
    if (IsChargedNitrogen(anAtom)) return NC;
    // check for donors
...
#ifdef _DEBUG1
        cout << _CT << " receptor type: " << PMFType2Str(theReceptorList[i]->GetPMFType())
...
RbtPMFType RbtSetupPMFSF::GetPMFfor_lN(RbtAtomPtr anAtom) {
    ...
    if (RbtAtom::SP == theHybState) return N0;  // N-zero and not en-oh
    ...
    if (RbtAtom::AROM == theHybState) return NR;
    // charged
    else if (IsChargedNitrogen(anAtom))
        return NC;
...
RbtPMFType RbtSetupPMFSF::GetPMFfor_lO(RbtAtomPtr anAtom) {
    ...
    RbtAtomList cbdAtomList = Rbt::GetAtomList(Rbt::GetBondedAtomList(anAtom), bIsC);
    if (cbdAtomList.size() > 1) return OE;
    // check for H-bond donor role
    RbtAtomList donorList = Rbt::GetAtomList(Rbt::GetBondedAtomList(anAtom), isHBondDonor);
    if (donorList.size() > 0) return OD;
