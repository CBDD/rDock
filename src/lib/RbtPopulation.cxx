...
void RbtPopulation::GAstep(
    ...
    for (RbtInt i = 0; i < nReplicates / 2; i++) {
        ...
        while (father == mother) {
            ...
            if (j > 100) throw RbtDockingError(_WHERE_, "Population failure - not enough diversity");
            j++;
        }
        ...
        if (m_rand.GetRandom01() < pcross) {
            Rbt::Crossover(father->GetChrom(), mother->GetChrom(), child1->GetChrom(), child2->GetChrom());
...
RbtGenomePtr RbtPopulation::Best() const { return m_pop.empty() ? RbtGenomePtr() : m_pop.front(); }
