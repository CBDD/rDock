...
RbtFilterExpressionPtr RbtParser::Parse(RbtTokenIterPtr ti, RbtContextPtr contextp) {
    ...
    if (t->IsLog()) return new FilterLogExp(op1);
    if (t->IsExp()) return new FilterExpExp(op1);
    RbtFilterExpressionPtr op2 = Parse(ti, contextp);
    if (t->IsAdd()) return new FilterAddExp(op1, op2);
    if (t->IsSub()) return new FilterSubExp(op1, op2);
    if (t->IsMul()) return new FilterMulExp(op1, op2);
    if (t->IsDiv()) return new FilterDivExp(op1, op2);
    if (t->IsAnd()) return new FilterAndExp(op1, op2);
    if (t->IsIf()) {
