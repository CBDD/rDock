                ...
                if (bImplHCorr) {
                        ...
                        for (RbtInt i1 = 0; i1 != offset2.size(); i1++) {
                            ...
                            if (iTrace > 1) {
                                cout << _CT << ": <H" << i1 + 1 << ">-" << pAtom2->GetAtomName() << "-"
                                     << pAtom3->GetAtomName() << "-" << pAtom4->GetAtomName()
                    ...
                    if (iter4 == bondedAtoms3.begin()) {
                        for (RbtInt i4 = 0; i4 != offset3.size(); i4++) {
                            ...
                            if (iTrace > 1) {
                                cout << _CT << ": " << pAtom1->GetAtomName() << "-" << pAtom2->GetAtomName() << "-"
                                     << pAtom3->GetAtomName() << "-" << pAtom4->GetAtomName()
                                     << "\toffset = " << dihprms.offset << endl;
                            }
                            pDih->AddTerm(dihprms);
...
RbtDihedral::prms RbtDihedralSF::FindDihedralParams(
    ...
    if (iTrace > 1) {
        cout << _CT << ": Assigned " << str1 << "," << str2 << "," << str3 << "," << str4 << "\tk=" << k << ", s=" << s
             << endl;
    }
...
void RbtDihedralSF::CalcBondedAtoms(
    ...
    if (nH == 0) return;
    if (GetTrace() > 2) {
        cout << _CT << ": determining impl H offsets for " << pAtom1->GetAtomName() << " - " << pAtom2->GetAtomName()
             << endl;
    }
