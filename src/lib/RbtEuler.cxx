...
RbtQuat RbtEuler::ToQuat() const {
    ...
    RbtQuat q(c1c2 * c3 - s1s2 * s3, c1c2 * s3 + s1s2 * c3, c1 * s2 * c3 - s1 * c2 * s3, s1 * c2 * c3 + c1 * s2 * s3);
    return q;
}
...
void RbtEuler::FromQuat(const RbtQuat& q) {
    ...
    if (test > 0.499999) {  // singularity at north pole
        ...
        m_heading = atan2(2.0 * (q.v.z * q.s - q.v.x * q.v.y), 1.0 - 2.0 * (sq.z + sq.y));
        m_attitude = asin(2.0 * test);
        m_bank = atan2(2.0 * (q.v.x * q.s - q.v.z * q.v.y), 1.0 - 2.0 * (sq.x + sq.y));
    }
}
