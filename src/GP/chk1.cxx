...
void main(int argc, char* argv[]) {
    try {
        ...
        if (!trainingFile) throw RbtError(_WHERE_, "can't open " + strTrainingFile);
        RbtString strTestFile = argv[1];
        strTestFile += "_testing.dat";
        fstream testFile(strTestFile.c_str(), ios::in);
        if (!testFile) throw RbtError(_WHERE_, "can't open " + strTestFile);
        ...
        while ((b < 0.9) && (i < 50000)) {
            ...
            if ((i % 500) == 0) {
                ...
                if (b > currentBest) {
                    ...
                    count++;
                cout << b << "\t" << ff->CalculateFitness(p.Best(), ittrain, sfttrain, hitlimit, false) << endl;
                if (count >= 7) break;
            }
            i++;
        }
        ...
        cout << "best with training: " << ff->CalculateFitness(p.Best(), ittrain, sfttrain, false) << "\n"
             << ff->CalculateFitness(p.Best(), ittrain, sfttrain, 0.0, false) << endl;
        cout << "best with test:     " << ff->CalculateFitness(p.Best(), ittest, sfttest, false) << "\n"
