...
RbtReturnTypeList RbtGPParser::Parse(RbtGPChromosomePtr chrom, RbtReturnTypeList& inputs) {
    // Give the values of the inputs to the corresponding cells
    for (RbtInt i = 0; i < nProgramInputs; i++) chrom->Cells(i)->SetResult(*(inputs[i]));
...
RbtString RbtGPParser::PrintParse1Output(RbtGPChromosomePtr chrom, RbtInt output, RbtBool numbers, RbtBool ins) {
    RbtInt startCell = (nFunctionsInputs + 1) * (output - nProgramInputs);
    RbtInt ncomm = (*chrom)[startCell + nFunctionsInputs];
    if (ncomm == IF) RbtCommand::ntabs++;
    RbtInt nargs = commands[ncomm]->GetNArgs();
    ...
    for (RbtInt i = 0; i < commands[ncomm]->GetNArgs(); i++) commands[ncomm]->SetNameArg(i, fs[i]);
    RbtString c;
...
RbtString RbtGPParser::PrintParse(istream& istr, RbtGPChromosomePtr chrom, RbtBool numbers, RbtBool ins) {
