...
RbtGPPopulation::RbtGPPopulation(
    ...
    for (RbtGPGenomeListIter iter = pop.begin(); iter != pop.end(); iter++) *iter = RbtGPGenomePtr(new RbtGPGenome());
...
RbtGPPopulation::RbtGPPopulation(const RbtGPPopulation& p): m_rand(Rbt::GetRbtRand()) {
    popsize = p.popsize;
    nrepl = p.nrepl;
    ff = p.ff;
    ittrain = p.ittrain;
    ...
    c = 2.0;  // default value for the sigma truncation multiplier
    pop = RbtGPGenomeList(popsize);
    for (RbtInt i = 0; i < p.popsize; i++) pop[i] = p.pop[i];
    newpop = RbtGPGenomeList(p.newpop.size());
    for (RbtUInt i = 0; i < p.newpop.size(); i++) newpop[i] = p.newpop[i];
    psum = new RbtDouble[popsize];
...
void RbtGPPopulation::Eval(RbtDouble hitlimit, RbtBool function) {
    for (RbtInt i = 0; i < popsize; i++) ff->CalculateFitness(pop[i], ittrain, sfttrain, function);
}
...
void RbtGPPopulation::GAstep(
    ...
    for (RbtInt i = 0; i < nrepl - 1; i++) {
        ...
        if (m_rand.GetRandom01() < pcross) newpop[i]->UniformCrossover(*mother, *father);
    ...
    *(newpop[nrepl - 1]) = *(pop[bestInd]);
    ...
    RbtDouble bestFitness = -FLT_MAX, fit;
    for (RbtUInt i = 0; i < newpop.size(); i++) {
        *(pop[i]) = *(newpop[i]);
        fit = ff->CalculateFitness(pop[i], ittrain, sfttrain, function);
        if (fit > bestFitness) {
            bestFitness = fit;
...
RbtGPGenomePtr RbtGPPopulation::RkSelect() const {
    ...
    index = max * (bias - sqrt(bias * bias - 4.0 * (bias - 1) * m_rand.GetRandom01())) / 2.0 / (bias - 1);
    if (index < 0) {
        cout << index << " " << max << endl;
        index = 0.0;
    }
