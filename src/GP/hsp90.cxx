...
void main(int argc, char* argv[]) {
    try {
        ...
        if (!inputFile) throw RbtError(_WHERE_, "can't open " + strInputFile);
        ...
        for (iter = randomList.begin(); iter != randomList.end(); iter++) cout << *iter << "\t";
        cout << endl;
        RbtString line;
        ...
        while ((b < 0.9) && (i < 10000)) {
            ...
            if ((i % 500) == 0) {
                ...
                //                                    hitlimit,false) << endl;
                cout << b << "\t" << ff->CalculateFitness(p.Best(), ittrain, sfttrain, false) << endl;
                if ((i == 5000) && (b < 0)) break;
            }
            i++;
        }
        cout << i << "\t" << *(p.Best()) << endl;
        cout << "best with training: " << ff->CalculateFitness(p.Best(), ittrain, sfttrain, false) << endl;
        cout << "best with test:     " << ff->CalculateFitness(p.Best(), ittest, sfttest, false) << endl;
        inputFile.seekg(0, ios::beg);
        ff->ReadTables(inputFile, ittest, sfttest);
        cout << "best with all:      " << ff->CalculateFitness(p.Best(), ittest, sfttest, false) << endl;
