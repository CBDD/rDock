...
int main(int argc, char *argv[]) {
    ...
    // Brief help message
    if (argc == 1) {
        cout << endl << "rbtether - marks the tethered atoms for use in docking" << endl;
        cout << endl
    ...
    try {
        ...
        if (tetherAtoms.empty()) {
            throw RbtError(_WHERE_, "SMARTS query should have at least one match in the reference SD file");
        }
        if (tetherAtoms.size() > 1) {
            // throw RbtError(_WHERE_,
            cout << "More than one SMARTS match found with the reference SD file" << endl;
