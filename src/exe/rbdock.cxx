...
void PrintUsage(void) {
    ...
    cout << "rbdock -i <sdFile> -o <outputRoot> -r <recepPrmFile> -p <protoPrmFile> [-n <nRuns>] [-ap] [-an] [-allH]"
         << endl;
    ...
    cout << "\t\t-c - continue if score threshold is met (use with -t <targetScore>, default=terminate ligand)"
         << endl;
...
int main(int argc, const char *argv[]) {
    ...
    if (!bStop)  // stop after target
        cout << " -cont " << endl;
    if (bTarget) cout << " -t " << dTargetScore << endl;
    ...
    if (!bFilter) {
        if (bTarget)  // -t<TS>
        {
            if (!bDockingRuns)  // -t<TS> only
            {
                ...
                {
                    strFilter << "1 if - SCORE.NRUNS " << (nDockingRuns - 1) << " 0.0 -1.0,\n1 - SCORE.INTER "
                              << dTargetScore << endl;
                } else  // -t<TS> -n<N>
                {
                    strFilter << "1 if - " << dTargetScore << " SCORE.INTER 0.0 "
                              << "if - SCORE.NRUNS " << (nDockingRuns - 1) << " 0.0 -1.0,\n1 - SCORE.INTER "
                              << dTargetScore << endl;
                }
    ...
    try {
        ...
        for (RbtInt nRec = 1; spMdlFileSource->FileStatusOK(); spMdlFileSource->NextRecord(), nRec++) {
            ...
            try {
                ...
                if (nDockingRuns < 1) bTargetMet = true;
                while (!bTargetMet) {
