...
void CheckAmideBonds(RbtModelPtr spModel) {
    ...
    for (RbtBondListConstIter bIter = amideBondList.begin(); bIter != amideBondList.end(); bIter++) {
        ...
        if ((oIter != bondedAtomsC.end()) && (hIter != bondedAtomsN.end())) {
            ...
            cout << spH->GetAtomName() << "(" << spH->GetFFType() << ") - " << spN->GetAtomName() << "("
                 << spN->GetFFType() << ") - " << spC->GetAtomName() << "(" << spC->GetFFType() << ") - "
                 << spO->GetAtomName() << "(" << spO->GetFFType() << "): " << phi << " deg" << endl;
            ...
            cout << spN->GetAtomName() << "(" << spN->GetFFType() << ") - " << spC->GetAtomName() << "("
                 << spC->GetFFType() << "): TERTIARY" << endl;
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        cout << endl
             << strExeName << " - output interaction center info for ligands in SD file (with optional autoionisation)"
             << endl;
    ...
    if (!bList) {
        cout << setw(8) << "RECORD" << setw(30) << "NAME" << setw(8) << "#ATOMS" << setw(8) << "#BONDS" << setw(8)
             << "MW" << setw(8) << "#ROT" << setw(8) << "#HBD" << setw(8) << "#HBA" << setw(8) << "#AROM" << setw(8)
             << "#GUAN" << setw(8) << "#LIPOC" << endl;
    }

    try {
        // Prepare the SD file sink for saving the autoionised structures for each ligand
        RbtMolecularFileSinkPtr spMdlFileSink;
        if (bWriteLigand) spMdlFileSink = RbtMolecularFileSinkPtr(new RbtMdlFileSink(strOutputSDFile, RbtModelPtr()));
        ...
        for (RbtInt nRec = 1; spMdlFileSource->FileStatusOK(); spMdlFileSource->NextRecord(), nRec++) {
            ...
            if (bList) {
                ...
                for (RbtBondListConstIter bIter = rotatableBondList.begin(); bIter != rotatableBondList.end();
                     bIter++) {
                    RbtAtomPtr spAtom1 = (*bIter)->GetAtom1Ptr();
                    RbtAtomPtr spAtom2 = (*bIter)->GetAtom2Ptr();
                    cout << spAtom1->GetAtomName() << "(" << spAtom1->GetFFType() << ") - " << spAtom2->GetAtomName()
                         << "(" << spAtom2->GetFFType() << ")" << endl;
                }
            } else {
                // Output the ligand info
                cout << setw(8) << nRec << setw(30) << strModelName.c_str() << setw(8) << nAtoms << setw(8) << nBonds
                     << setw(8) << dMolWt << setw(8) << nROT << setw(8) << nNHBD << setw(8) << nNHBA << setw(8)
