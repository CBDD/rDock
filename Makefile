...
#   targets: display a list of all the targets available with a short description
...
#   test_rbcavity: runs rbcavity on the test systems to generate the .as files
...
#         and the data folder in RBT_ROOT (see below)
#   full: cleans everything and rebuilds the library and binaries from scratch,
#         then runs the tests and uses the install target to install everything
...
#			binaries are installed in $(PREFIX)/bin, library in $(PREFIX)/lib,
#			and headers in $(PREFIX)/include/rdock
#
#   RBT_ROOT: the folder where the rDock data folder is installed. Default: $(PREFIX)/share/rdock
#
#   CXX: the c++ compiler to use. Default: g++
#
#   CXX_EXTRA_FLAGS: extra flags to pass to the compiler. Default: empty
#
#   CXX_STD: the c++ standard to use. Default: c++17
#
#   RBT_VERSION: the version of the library. Default: vYY.MM-alpha
#           this is used by the CI/CD pipeline to set the version of the library and binaries
#           you can set it to any value you want for your local builds, for cosmetic purposes.

PREFIX                      ?= /usr
RBT_ROOT                    ?= $(PREFIX)/share/rdock
CONFIG                      ?= RELEASE
...
install-include: target_folders
	@cp -r include/* $(PREFIX)/include/rdock
...
target_folders:
	@mkdir -p $(PREFIX)/bin $(PREFIX)/lib $(PREFIX)/include/rdock $(RBT_ROOT)
